...
export abstract class RestrictedGatewayAdapter implements CodestraConnector {
  ...
  async execute(
    ...
    if (typeof commandId !== "string") {
      throw new ConnectorError("Restricted gateway omitted commandId.", "INVALID_PROVIDER_RESPONSE");
    }
    return {
      commandId,
      status,
  ...
  async ingestWebhook(
    ...
    return this.webhookNormalizer(context, input);
  }
  ...
  async reconcile(context: ConnectorContext, cursor?: string): Promise<ReconciliationResult> {
    ...
    const items = response.items.filter(isJsonObject);
    return {
      items,
