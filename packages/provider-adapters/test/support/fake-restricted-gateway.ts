 ...
 * RestrictedGatewayAdapter's real HTTP behavior (timeouts, non-2xx
 * handling, connection resets, redirects) against this stand-in rather
 * than mocking `fetch`. Mirrors services/middleware/test/support's helper
 * of the same name, which does the same for that service's independent
 * RestrictedGatewayClient implementation.
 */
