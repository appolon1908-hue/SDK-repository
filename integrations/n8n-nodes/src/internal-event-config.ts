...
export function parseInternalWebhookConfig(
  credentials: InternalCredentialData,
): CodestraInternalWebhookConfig {
  const expectedTenantId = requireUuid(requireCredentialString(credentials.expectedTenantId, "expectedTenantId"), "expectedTenantId");
  const webhookSecrets = parseLines(credentials.webhookSecrets, "webhookSecrets");
  if (webhookSecrets.length === 0) {
    throw new InternalEventBoundaryError("At least one webhook signing secret is required.", "WEBHOOK_SECRETS_REQUIRED", {
      status: 500,
    });
  }
  ...
  if (allowedEventTypes.size === 0 || allowedSourcePrefixes.length === 0) {
    throw new InternalEventBoundaryError(
      "Event type and source allowlists must not be empty.",
      "EVENT_ALLOWLIST_REQUIRED",
      { status: 500 },
    );
  }
  ...
  return {
    ...
    replayGuardBaseUrl: validateInternalBaseUrl(
      requireCredentialString(credentials.replayGuardBaseUrl, "replayGuardBaseUrl"),
    ),
    replayGuardAccessToken: requireSecret(
      requireCredentialString(credentials.replayGuardAccessToken, "replayGuardAccessToken"),
      "replayGuardAccessToken",
    ),
    timestampToleranceSeconds: integerBetween(
      credentials.timestampToleranceSeconds ?? 300,
      0,
      86_400,
