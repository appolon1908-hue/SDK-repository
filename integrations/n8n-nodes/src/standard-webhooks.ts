...
export async function signWebhook(input: SignWebhookInput): Promise<WebhookHeaders> {
  ...
  const signature = await computeSignature(input.secret, signedPayload(id, timestamp, input.payload));
  return {
    "webhook-id": id,
...
export async function verifyWebhook(input: VerifyWebhookInput): Promise<VerifiedWebhook> {
  ...
  const body = signedPayload(id, timestamp, input.payload);
  for (let index = 0; index < input.secrets.length; index += 1) {
    const secret = input.secrets[index];
    if (secret === undefined) continue;
    const expected = fromBase64(await computeSignature(secret, body));
...
async function computeSignature(secret: string, payload: Uint8Array): Promise<string> {
  if (!globalThis.crypto?.subtle) {
    throw new WebhookVerificationError("Web Crypto HMAC support is required.", "CRYPTO_UNAVAILABLE");
  }
  const key = await globalThis.crypto.subtle.importKey(
    "raw",
    Uint8Array.from(decodeSecret(secret)).buffer,
    { name: "HMAC", hash: "SHA-256" },
    false,
    ["sign"],
  );
  const signature = await globalThis.crypto.subtle.sign("HMAC", key, Uint8Array.from(payload).buffer);
  return toBase64(new Uint8Array(signature));
}

function signedPayload(id: string, timestamp: number, payload: string | Uint8Array): Uint8Array {
...
function parseSignatures(header: string): Uint8Array[] {
  ...
  for (const token of header.trim().split(/\s+/u)) {
    const [version, value, extra] = token.split(",");
    if (version !== "v1" || !value || extra !== undefined) continue;
    try {
      signatures.push(fromBase64(value));
    } catch {
      // Ignore malformed rotated signature candidates.
    }
  }
